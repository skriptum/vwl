...
```{r}
#credentials saved via save_credentials()
...
Regionalstatistik [AI](https://www.regionalstatistik.de/genesis//online?operation=table&code=AI-S-04&bypass=true&levelindex=0&levelid=1705078025766#abreadcrumb)

take d from question before, it is the same
```{r}
data <- retrieve_data(tablename="99910KJA21", genesis=cred)
head(data)
```
...
hartz_data <- data %>% 
  filter(JAHR == 2021) %>%
  select(kreis = KREISE, 
         hartz = AI2102_val #hartz IV rate in %
         ) %>%
  # remove rows with more than 5 digits in kreis
  filter(nchar(kreis) == 5)
```
...
```{r}
wahl_lohn <- wahl_lohn %>%
  left_join(hartz_data, by = "kreis")
```

```{r}
model_hartz <- lm(
    afd_prozent ~ lohn_prozent + ost + arbeitslosenquote + log(gdp) + log(pop) + hartz, 
    data = wahl_lohn
  )
summary(model_hartz)
```
